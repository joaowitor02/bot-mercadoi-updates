...
If Not objFSO.FileExists(strFlag) Then
    ...
    fBat.WriteLine "echo."
    fBat.WriteLine "echo  [1/2] Instalando dependencias do bot..."
    fBat.WriteLine """" & strPython & """ -m pip install -r """ & strDir & "\requirements.txt"" -q >> """ & strLogs & "\setup.log"" 2>&1"
    fBat.WriteLine "if errorlevel 1 (echo  ERRO: falha ao instalar dependencias. Veja logs\setup.log) else (echo  [1/2] OK)"
    fBat.WriteLine "echo."
    fBat.WriteLine "echo  [2/2] Baixando navegador interno (pode demorar 2-3 minutos)..."
    fBat.WriteLine """" & strPython & """ -m playwright install chromium >> """ & strLogs & "\setup.log"" 2>&1"
    fBat.WriteLine "if errorlevel 1 (echo  AVISO: falha ao instalar navegador) else (echo  [2/2] OK)"
    fBat.WriteLine "echo."
    fBat.WriteLine "echo  ======================================================"
